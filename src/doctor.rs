//! doctor.rs — System & Environment Diagnostics for aegis.
...
fn check_binary() -> DoctorCheck {
    let exe = env::current_exe().unwrap_or_else(|_| PathBuf::from("aegis"));
    if !exe.exists() {
        return DoctorCheck {
            name: "binary_integrity".into(),
            status: "error".into(),
            detail: format!("Binary not found at {}", exe.display()),
            remediation: Some("Reinstall aegis via install.sh".into()),
        };
    }
